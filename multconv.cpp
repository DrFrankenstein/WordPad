...
BOOL CConverter::IsFormatCorrect(LPCTSTR pszFileName)
{
	...
	char buf[_MAX_PATH];
	strcpy_s(buf, MAX_PATH, T2CA(pszFileName));
	...
	CFileException* pException)
{
	...
	CharToOemBuffA(buf, buf, nLen);
	OemToCharBuffA(buf, buf, nLen);
...
UINT CConverter::Read(void FAR* lpBuf, UINT nCount)
{
	...
	while (cch != 0)
	{
		...
		memcpy_s(pBuf, nCount, m_pBuf, nBytes);
		pBuf += nBytes;
		m_pBuf += nBytes;
...
ULONGLONG CConverter::Seek(LONGLONG lOff, UINT nFrom)
{
...
void CConverter::UnlockRange(ULONGLONG, ULONGLONG)
{
	AfxThrowNotSupportedException();
}

void CConverter::SetLength(ULONGLONG)
{
	AfxThrowNotSupportedException();
}
